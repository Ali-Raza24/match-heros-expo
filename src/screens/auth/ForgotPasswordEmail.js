 ...
 class ForgotPasswordEmail extends Component {
  ...
  handleSubmit() {
    ...
    if (/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(this.state.email)) {
      ...
      this.authService.resetPasswordEmail({ email: this.state.email })
        .then(res => {
          this.setState({ emailSuccess: res.data.message, loading: false });
        })
        .catch(err => {
          this.setState({ emailError: 'There is no user with that email address.', loading: false });
        })
    } else {
  ...
  <StatusBar backgroundColor="#1E2646" barStyle= {'light-content'}/>
      ...
      >
        ...
        <View style={{ marginTop: 20 }}>       
          ...
          <TextInputField placeHolder={"Enter email address"} placeHolderColor={"#ffffff"} inputFieldBackColor= {"#1E2646"} inputColor='#ffffff' borderBottomColor={'#1E2646'}
           onSubmitEditing={() => {
          ...
          <GreenLinearGradientButton title={"SEND"} onSelect={() => this.props.navigation.goBack()} height={45}
