...
export const authUser = () => {
  return async (dispatch, getState) => {
    ...
    if (token) {
      authService.get()
        .then(res => {
          authService.getUserTokens(res.data.user.id)
            .then(response => {
              //  Set tokens actions
              dispatch({
      ...
      authService.get()
        .then(response => {
          ApiService.setDefaultHeaders(response.data.authorization.access_token);
          //  Get user tokens
          authService.getUserTokens(response.data.user.id)
            .then(res => {
              //  Set tokens actions
  ...
  return async (dispatch) => {
    ...
    const loginResponse = await authService.login(values)
      .then(async (response) => {
        ...
        // Set token in memory
        await apiService.setDefaultHeaders(response.data.authorization.access_token);
        //  Set token in storage
        await AsyncStorage.setItem("userToken", response.data.authorization.access_token);
        //  Get user tokens
...
export const updateUser = (data) => {
  return async (dispatch) => {
    ...
    //  Get user tokens
    authService.getUserTokens(data.user.id)
      .then(res => {
        //  Set tokens actions
        dispatch({
          type: UPDATE_USER_TOKENS,
